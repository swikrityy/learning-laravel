...
@section('content')
    <div class="card mb-4">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h5 class="mb-0">Payment Methods ({{ $count }})</h5>
                    ...
                    <table class="table" id="dtable">
                        ...
                        <tbody class="table-border-bottom-0">
                            ...
                            @foreach ($payments as $key => $payment)
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    <td><img src="{{ asset($payment->image) }}" width="80px" alt="{{ $payment->name }}">
                                    </td>
                                    <td>{{ $payment->title }}</td>
                                    <td>{{ $payment->order }}</td>
                                    <td>
                                    ...
                                    <td class="gap-2">
                                        <a href="{{ route('payment.edit', $payment->id) }}" type="button"
                                        ...
                                        <form action="{{ route('payment.destroy', $payment->id) }}" method="post"
                                            ...
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-sm btn-icon btn-danger delete_payment">
