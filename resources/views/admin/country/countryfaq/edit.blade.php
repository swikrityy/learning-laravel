...
@section('content')
    <div class="card mb-4">
        <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <small class="text-muted float-end">
                <a href="{{ route('countryfaq.index', ${$name}->country->id) }}"
    ...
    <div>
        <form action="{{ route('countryfaq.update', ${$name}->id) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                        ...
                        <div class="card-body">
                            ...
                            <div class="mb-4">
                                ...
                                <textarea class="form-control ckeditor" id="answer" name="answer" placeholder="Answer"
                                    rows="4">{{ old('answer', ${$name}->answer) }}</textarea>
                                @error('answer')
                                    <p class="text-danger">{{ $message }}</p>
                                @enderror
                            </div>
